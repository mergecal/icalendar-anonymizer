...
- Added core anonymization engine with :py:func:`anonymize` function that removes personal data (names, emails, locations, descriptions) while preserving technical properties (dates, recurrence, timezones). Implemented hash-based deterministic anonymization with configurable salt parameter. Added property classification system distinguishing preserved vs anonymized properties with default-deny for unknown properties. Added structure-preserving hash functions maintaining word count and email format. Implemented UID uniqueness preservation and special handling for ``ATTENDEE``/``ORGANIZER`` with ``CN`` parameter anonymization. Added comprehensive test suite with 40+ tests covering property preservation, anonymization, UID handling, and edge cases. Added :file:`src/icalendar_anonymizer/anonymizer.py`, :file:`src/icalendar_anonymizer/_properties.py`, :file:`src/icalendar_anonymizer/_hash.py`, and :file:`src/icalendar_anonymizer/tests/lib/test_anonymization.py`. See `Issue 1 <https://github.com/mergecal/icalendar-anonymizer/issues/1>`_ and `Issue 2 <https://github.com/mergecal/icalendar-anonymizer/issues/2>`_.
